...
TEST(WebSocketSharedTest, DetectsSessionTarget) {
  ...
  EXPECT_FALSE(IsSessionWebSocketTarget("/ws/sessions/s_1/controller"));
  EXPECT_FALSE(IsSessionWebSocketTarget("/sessions/s_1"));
}

TEST(WebSocketSharedTest, DetectsControllerTarget) {
  EXPECT_TRUE(IsControllerWebSocketTarget("/ws/sessions/s_1/controller"));
  EXPECT_TRUE(IsControllerWebSocketTarget("/ws/sessions/s_1/controller?access_token=abc"));
  EXPECT_FALSE(IsControllerWebSocketTarget("/ws/sessions/s_1"));
}
...
TEST(WebSocketSharedTest, ExtractsSessionId) {
  ...
  EXPECT_EQ(ExtractSessionIdFromWebSocketTarget("/ws/sessions/s_123/controller"), "s_123");
...
TEST(WebSocketSharedTest, ExtractsAccessToken) {
  ...
  EXPECT_EQ(ExtractAccessTokenFromWebSocketTarget("/ws/sessions/s_123/controller?access_token=xyz"), "xyz");
